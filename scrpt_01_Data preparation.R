...
data <- read.csv("./Data_Morphology.csv")
str(data)
...
func.names <- c("size", "shape", "nas.gls", "can.bam", "pl.cb", "bam.pl", "sm.max", "alpcor.mad")
...
## Export results
write.csv(t(tab), "./Results/Results_Measur-error.csv")
...
matrix <- matrix(NA, 1, length(trait.names))
colnames(matrix) <- trait.names ; rownames(matrix) <- "Pr(>F)"
...
# Export the results
write.csv(matrix, "./Results/Results_Sex-dimorph-anova.csv")
...
# Create a matrix for the result
matrix <- data.frame(trait=trait.names)

df <- droplevels(df)
pop <- data.frame(sex.pop=paste("female", levels(df$pop), sep="."))
pop <- rbind(pop, data.frame(sex.pop = paste("male", levels(df$pop), sep = ".")))
...
for(i in levels(df$pop)){
  for(t in trait.names){
    y.female <- subset(df, pop == i & sexo == "female") 
    y.male <- subset(df, pop == i & sexo == "male") 
    
    pop[which(pop$sex.pop == paste("female",i, sep=".")),t] <- mean(y.female[,t])
    pop[which(pop$sex.pop == paste("female",i, sep=".")),"sexo"] <- "female"
    pop[which(pop$sex.pop == paste("male",i, sep=".")),t] <- mean(y.male[,t])
    pop[which(pop$sex.pop == paste("male",i, sep=".")),"sexo"] <- "male"
  }
  pop[which(pop$sex.pop == paste("female",i, sep=".")),"size"] <- mean(y.female[,"size"])
  pop[which(pop$sex.pop == paste("male",i, sep=".")),"size"] <- mean(y.male[,"size"])
}
...
# Export results
write.csv(matrix, file = "./Results/Results_Sex-dimorph-shape.csv")


## 2.4. FIGURE: SEXUAL DIMORPHISM ####
source("./function_Corner_text.R")
...
write.csv(df, "./Data_Size-corrected.csv")
...
df <- read.csv("./Data_Size-corrected.csv")
...
for(i in trait.names){
  ...
  hist(t, breaks=10, prob=TRUE, col="grey",
...
for(i in func.names){
  ...
  hist(t, breaks=10, prob=TRUE, col="grey", xaxt="n", yaxt="n",
       main=paste(i), xlab="", ylab="", axes=TRUE) 
  lines(density(t), col="red", lwd=2)
}
...
for(i in trait.names){
  ...
  qqnorm(t, main=paste(i)) ; qqline(t)
}
